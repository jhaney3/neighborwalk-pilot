...
Updated September 10, 2026. This is an acceptance map for the [original audit](market-readiness-audit.md) and [approved plan](market-readiness-plan.md), not a replacement scope or a production-readiness certificate. Exact commits, failures and completed checks are in the [execution ledger](rework-progress.md).
...
| B02 — Durable restrictions | Independent restrictions enforced by the server; later encounters do not lift them; reviewed correction preserves history. |
...
| B14 — Production hardening | Local/hosted test foundation, session security and headers implemented; monitoring, staging/cutover, live delivery, operator runbooks and runtime verification remain open. |
...
| --- | --- |
| I01 — Outings | Routed lifecycle, reusable areas/groups, assignment acknowledgement, readiness, debrief and repeat drafts. Real leader preparation-time/usability measurement remains. |
...
| I07 — Restrictions and tasks | Independent channel restrictions and server-side effects, including protected responsibilities unavailable to the initiating volunteer. |
...
| I14 — Church metadata | Canonical relational settings and derived presentation, with transactional version/revision checks; legacy snapshot is preserved rather than dual-written. |
| I15 — Competing design contracts | Current architecture/types/migrations documented; old SQL/OpenAPI explicitly archived. README no longer presents the old design or single deploy command as a safe release. |
