...
Implemented on `codex/neighborwalk-ios`. The authorized Supabase and public-policy rollout is recorded in `docs/ios-release.md`; Apple Developer configuration remains pending. The iOS styling is committed here. The invitation migration retains the website's original email-bound invitation functions.
...
- The deployed `account-deletion` Edge Function implements native reauthorization, server-side authorization-code exchange, verified identity binding and Apple token revocation. Its fulfillment migration is deployed; supply the server-only Apple key configuration listed in [the release guide](ios-release.md). This does not perform account/content erasure or send confirmation. Complete the operator fulfillment rehearsal and signed-device Apple/Hide My Email tests before release; no production activation is claimed.

## Enable invitation links without modifying the hosted website

1. Rehearse the additive migration `supabase/migrations/20260919214330_flexible_church_invitations.sql` against a development Supabase project containing all prior migrations. `npm run test:invitations` exercises the actual SQL in isolated PostgreSQL with a minimal prior-schema fixture. The full migration sequence also passed a local rollback-only rehearsal; repeat the release candidate in staging before production rollout.
2. This additive migration is deployed; see the hosted/source version mapping in the release guide. It adds private tables and authenticated RPCs; it does not change legacy invitation behavior or membership policies. Creation/revocation retain live-leader and recent-auth checks. Tokens are 256-bit random capabilities stored only as hashes; public invitation URLs put tokens in fragments.
