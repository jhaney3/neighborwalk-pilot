...
**Not submitted to TestFlight or App Review.** The intended listing is **SendMe: Gospel Outreach**, distributed as an unlisted App Store app by direct link. App Store Connect app `6816471876` exists with version 1.0 in Prepare for Submission and manual release selected. Apple must separately approve unlisted distribution; keep the app on manual release while that request is pending.

Signed Release archives and App Store IPA exports have succeeded on this Mac. A development-signed build is installed on the connected iPhone 15 Pro, and the user confirmed that native Apple sign-in reached the workspace/invitation screen. This is one verified authentication flow, not complete device acceptance. The completed app/haptics/entry-screen edits and MapTiler correction pass the current source checks and are installed on the iPhone; the user also confirmed building outlines in the original problem area. The fresh archive `outputs/ios/SendMe-ReleaseCandidate.xcarchive` and export `outputs/ios/app-store-candidate/App.ipa` include that tested snapshot. Strict code-signature verification, production APNs entitlement, disabled debugging, App Store provisioning, native map identifier and bundled-asset equality checks passed. The IPA SHA-256 is `a8f289bec7b8309dcf46e8dfeded3ca961d737881e32d29b8c3847ae4743f2e2`. It has not been uploaded.

Remaining release gates:

1. Recover the production APNs private-key download, configure the delivery worker and protected scheduler, and prove actual delivery on a distribution-signed build. Registering a key or deploying the function alone does not activate push.
2. Rehearse actual account/content erasure and completion confirmation using dedicated test records. Apple authorization revocation is implemented/configured; complete deletion fulfillment is not yet proven.
3. Enforce the approved 30-day maximum retention on the operator's backup copies on the other personal laptop, verify recovery, and finalize the public policy pages. Draft notices remain intentionally visible until the operational claims are established.
4. Complete App Store screenshots, reviewer access, privacy answers, age rating, distribution/pricing details, and final signed-device acceptance. Submit the reviewed build, then request unlisted distribution. Apple controls review timing.

## Mac rebuild and SendMe rename — September 26, 2026

- Updated the iOS checkout to `fbbdfa0` and installed dependencies with Node 22.23.3. **SendMe** is the approved user-facing name for the app, invitations and public website. Stable bundle identifiers, URL schemes and storage keys stay unchanged.
- Apple team `RXRH5R33FV` owns `app.neighborwalk.ios`, described as SendMe, with Push Notifications, Associated Domains and Sign in with Apple enabled. Automatic signing produced valid Release archives and App Store exports with production APNs entitlement; none has been uploaded.
- The separate invitation host is `https://neighborwalk-invites.vercel.app`. `/invite` and the Apple association file respond successfully without a redirect. Deployment: `dpl_58AHPoGAbxwcGq3XQbageddboe6i`. The native Associated Domain uses that host.
- Production website source is `c43ac3c` on `codex/sendme-web-release`, deployed as `dpl_AMx14fkWXGEwbGFyFtztQYCyWD1U` at https://neighborwalk-pilot.vercel.app. It contains the approved public identity (Jacob Haney; jacobbhaney@icloud.com), SendMe branding and manual backup-retention copy. Draft policy notices remain. Focused shared changes are also ported to the web development branch; no full branch merge was performed.
- Native Apple sign-in is enabled for `app.neighborwalk.ios` in Supabase. The separate Apple deletion key is configured server-side. The native email callbacks `neighborwalk://auth` and `neighborwalk://auth\?**` are saved alongside the existing redirects. The SMTP display name is SendMe. Native Apple sign-in passed the user's physical iPhone check; other auth/deletion flows still need acceptance.
- Supabase `push-delivery` version 2 and `account-deletion` version 1 are deployed. The unused APNs key `QD3D4ZDD3P` was revoked with approval. Its replacement `Z422LL962K` is production-only and restricted to the app topic, but the browser marked it downloaded without a recoverable local `.p8` file. Do not claim APNs secrets, scheduling or delivery are active.
- Scheduler extensions are installed: local migration `20260926171000_enable_push_scheduler` maps to hosted version `20260926170533`. `scripts/sql/activate-push-scheduler.sql` is prepared but has **not** been run; its Vault values and APNs/worker setup remain pending. Preserve this mapping alongside the five earlier migrations below.
- Jacob Haney agreed to personally handle deletion requests within the disclosed process. Manual backups must expire within 30 days of creation. The existing backup is on another personal laptop; retention enforcement and recovery have not been verified here. The Supabase organization remains on Free. Apple does not require buying Supabase Pro, so the user's conditional purchase authorization was not exercised.
- The native build lacked a MapTiler key and used the fallback map. The website's origin-restricted key rejects native requests. MapTiler's key limit blocked creating another key, so the previously unused default key is now named **SendMe iOS**, restricted to User-Agent `SendMe-iOS/app.neighborwalk.ios`, and configured only in the ignored mobile environment. The website key is unchanged. Direct native-identified style, building-tile, font and geocoding requests returned HTTP 200; an unrelated identifier returned 403. The worker-header correction avoids a rejected CORS preflight. The user confirmed that streets and building shapes appear in the simulator, then confirmed that outlines appear in the original problem area on the updated physical iPhone. Source fix: `dfb97e2`.
- Verification before the latest edits: lint/typecheck, 427 unit tests, sample/production builds, native sync/bundle provenance, simulator compilation, Release archive and App Store export passed. The first MapTiler request change passed lint/typecheck and 23 focused tests; the subsequent worker-header correction passed lint/typecheck, 24 focused tests, all 437 tests across 90 files, a sample build, production sync/bundle verification, and native Debug simulator/Release device builds. These results do not replace final checks after further concurrent edits.
- Both web source checkouts separately passed lint/typecheck, focused policy/retention tests and production builds. Public policy rendering was inspected. Mac Git authentication is configured. Tested app/haptics/entry-screen commits, map fix `dfb97e2`, release status, and both shared web-retention ports have been pushed to their existing branches.
- `ios:release:check` passes public configuration, signing metadata and live invitation/AASA checks. Public privacy/help/terms draft notices still block release preparation. Do not suppress these gates to obtain a passing result.

The following dated Linux preparation and audit sections preserve earlier evidence. Their outstanding-state descriptions are historical; the current status above takes precedence.
