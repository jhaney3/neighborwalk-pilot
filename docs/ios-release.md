...
Signed Release archives and App Store IPA exports have succeeded on this Mac. A development-signed build is installed on the connected iPhone 15 Pro, and the user confirmed that native Apple sign-in reached the workspace/invitation screen. This is one verified authentication flow, not complete device acceptance. The completed app/haptics/entry-screen edits and MapTiler correction pass the current source checks and are installed on the iPhone; the user also confirmed building outlines in the original problem area. A fresh App Store archive/export is still required from this snapshot.

Remaining release gates:

1. Export and verify the final archive from the tested source snapshot, including the map correction and completed app/haptics/entry-screen changes.
2. Recover the production APNs private-key download, configure the delivery worker and protected scheduler, and prove actual delivery on a distribution-signed build. Registering a key or deploying the function alone does not activate push.
3. Rehearse actual account/content erasure and completion confirmation using dedicated test records. Apple authorization revocation is implemented/configured; complete deletion fulfillment is not yet proven.
4. Enforce the approved 30-day maximum retention on the operator's backup copies on the other personal laptop, verify recovery, and finalize the public policy pages. Draft notices remain intentionally visible until the operational claims are established.
5. Complete App Store screenshots, reviewer access, privacy answers, age rating, distribution/pricing details, and final signed-device acceptance. Submit the reviewed build, then request unlisted distribution. Apple controls review timing.
...
- Both web source checkouts separately passed lint/typecheck, focused policy/retention tests and production builds. Public policy rendering was inspected. Mac Git authentication is configured and earlier release commits were pushed; check each branch's current ahead/behind state before the final push.
