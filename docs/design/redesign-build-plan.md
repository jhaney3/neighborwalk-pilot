...
### Entry screens and a map fix (2026-09-26)

75. **The screens before a church opens now use the redesign's language,** at the user's request. They were never drawn, so they follow the nearest locked patterns: paper background, the app-icon pin on a hedge tile, a left-aligned screen title over a mono line, sheet inputs with mono labels, `button-ink` / `button-outline` (the Settings family), grouped rows like More, and offset cards for the account and "Check your inbox". Screens EN1–EN16 are in `docs/design/entry-screens/` (before/, built/, built-dark/; `capture.mjs`, with `harness.tsx` for states no flow reaches). **Please review:**
    - Sign-in keeps Apple, then Google, at the top (Apple's HIG: never smaller than other sign-in buttons, visible without scrolling, black or white only; corners now 14pt to match the ink button). "More options" is gone: the sign-in link, phone code and invitation link are always visible under **More ways in**, the last two opening in place.
    - Problems (can't check sign-in, couldn't open your church, offline, crash, not connected) share one layout: a round icon, the title, what happened, a mono reassurance, then actions with the primary first. Loading is only the brand tile and a mono status line.
    - Copy was tightened, not changed in meaning: e.g. "Your prepared workspace is available" → "You're offline", "Open prepared offline workspace" → "Open saved records", "Ask your leader for an invitation link." → "Ask your leader for an invite".
    - Delete account is an outline button with red text beside Sign out (Settings and the no-church screens), and its sheet uses the ink/outline buttons with the destructive action in red. Every modal heading now uses the display face.
    - The Apple logo is still the app's own SVG path; Apple asks for artwork from Apple Design Resources. Swap it before release.
76. **The map header no longer overflows with a long neighborhood name.** `.map-top` was a grid with an auto column, so "Sunday Night Evangelism" (and the chip row) widened it past the screen, pushing the Walks / Map / List switch and ⋯ off the edge. It now has one `minmax(0, 1fr)` column and the name truncates; the neighborhood sheet shows it in full.
