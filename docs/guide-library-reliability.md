...
These checks do not establish broad production-shaped load capacity, physical-phone usability, authenticated production guide workflows or complete disaster recovery. Invalid legacy content requires supervised review without deleting the original. The connected guide API and matching application are deployed together; future changes must preserve that paired-release boundary and must not ship a direct-write revocation without its compatible client.
