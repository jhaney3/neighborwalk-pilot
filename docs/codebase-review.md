...
| High | Snapshot merge reconciles entities, but protected-table writes and the snapshot update are separate operations. Two edits to one person or property can still overwrite fields; no-op sync also does not fetch other devices' changes. See `runSync` and `workspace-sync.ts`. | Add tests with two authenticated devices, explicit same-record conflict rules, and a deliberate remote-refresh strategy before larger group use. |
