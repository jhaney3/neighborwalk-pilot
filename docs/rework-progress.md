...
| 1 — Safer baseline | Implemented locally; release verification incomplete | Patched dependencies, regression coverage, scoped storage and local database CI job; clean-install/CI and production verification remain |
| 2 — Durable domain, permissions and migration | Implemented locally in substantial part; not released | Eleven additive migrations, transactional commands, scoped recovery, reviewed administration and rollback-only preservation rehearsal; full relationship reconciliation, staging/cutover and failure matrix remain |
...
| 4 — Website and controlled pilot | Implemented locally in part; not ready for enrollment | Public site, CSV exchange, admin review and leader setup guide; reminders, operations, owner details, commercial hosting and external pilot gates remain |
