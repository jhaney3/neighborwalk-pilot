...
function page(content: string, status = 200) {
  return new Response('<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>NeighborWalk reminders</title><main><h1>NeighborWalk reminders</h1>' + content + '</main></html>', { status, headers });
}
...
export function GET(request: Request) {
  ...
  if (!preferenceId) return page("<p>This link is invalid. Open Settings in your signed-in NeighborWalk app to turn reminders off.</p>", 400);
...
export async function POST(request: Request) {
  ...
  return page("<p>Email reminders are turned off. A message already in flight may still arrive. You can opt in again from Settings in your signed-in NeighborWalk app.</p>");
}
