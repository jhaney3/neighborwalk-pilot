...
- **Haptics (`@capacitor/haptics`):** use them sparingly and consistently. Today only tab changes use them (`Haptics.selectionChanged()` in `mobile/main.tsx`). Suggested: a light impact when a pin drops, a success notification when a visit saves, and selection feedback on segmented controls and chips. Tell the user if you add others.
