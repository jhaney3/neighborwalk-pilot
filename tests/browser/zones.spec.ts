...
async function installTargetFixture(page: Page): Promise<TargetFixture> {
  ...
  // Browser-side code cannot read module imports; pass the demo center in.
  const fixture = await page.evaluate(async ({ idSuffix, demoCenter, testCenter }) => {
    ...
    for (const databaseName of databaseNames) {
      ...
      try {
        ...
        data.walkTargets.push(
          { id: firstTargetId, churchId: data.church.id, eventId, territoryId: parent.id, name: firstTargetName, color: "#286c59", selectionKind: "rectangle", geometry: rectangle(testCenter[1], testCenter[1] + 0.0024), parcels: [targetParcel(firstParcel)], rosterState: "frozen", frozenAt: now.toISOString() },
          { id: secondTargetId, churchId: data.church.id, eventId, territoryId: parent.id, name: secondTargetName, color: "#a9660d", selectionKind: "rectangle", geometry: rectangle(testCenter[1] - 0.0024, testCenter[1] - 0.0001), parcels: [targetParcel(apartmentParcel), targetParcel(remainingParcel)], rosterState: "frozen", frozenAt: now.toISOString() },
        );
        ...
        data.properties.push(
          property(`property_zone_browser_${idSuffix}_north`, outsideAddress, undefined, firstParcel, [center[0], testCenter[1] + 0.0009]),
          property(apartmentA, apartmentAddress, "Apartment A", apartmentParcel, [center[0] - 0.0002, testCenter[1] - 0.0012]),
          property(apartmentB, apartmentAddress, "Apartment B", apartmentParcel, [center[0] - 0.0001, testCenter[1] - 0.0012]),
...
test("a leader can ready a whole-zone walk and start it from the card", async ({ page }) => {
  ...
  const creator = dialog.locator(".walk-parent-zone-creator");
  const createZone = creator.getByRole("button", { name: "Create neighborhood", exact: true });
  ...
  await expect(creator.getByText("6 homes loaded. Draw around the neighborhood.", { exact: true })).toBeVisible();
  const bounds = await map.boundingBox();
  expect(bounds).not.toBeNull();
...
test("the planner starts from a persistent zone and offers area and street target modes", async ({ page }) => {
  ...
  const map = planner.getByRole("application", { name: "Route map", exact: true });
  const bounds = await map.boundingBox();
  expect(bounds).not.toBeNull();
...
test("real Lawrence parcels support a new zone and ready whole-zone walk without hosted writes", async ({ page }) => {
  ...
  await expect(creator.getByText(`${viewportPayload.features?.length} homes loaded. Draw around the neighborhood.`, { exact: true })).toBeVisible();
...
test("replacing a frozen target preserves history and requires fresh acceptance", async ({ page }) => {
  ...
  await expect(firstAssignment).toContainText("No team yet");

  const previousAssignment = page.locator(".assignment-list li").filter({ hasText: fixture.secondTargetName });
  await previousAssignment.getByRole("button", { name: "Replace route", exact: true }).click();
  const dialog = page.getByRole("dialog", { name: "Replace route", exact: true });
  await expect(dialog).toContainText(fixture.secondTargetName);
  const planner = dialog.getByRole("region", { name: `Routes in ${fixture.parentName}`, exact: true });
  await planner.getByRole("button", { name: "Whole zone", exact: true }).click();
  await expect(planner.locator(".walk-target-list li")).toHaveCount(1);
  await expect(planner.getByRole("button", { name: /^Remove / })).toBeEnabled();
  ...
  await expect(previousAssignment).toContainText("No team yet");
  ...
  await expect(replacementAssignment.getByRole("button", { name: "Going", exact: true })).toHaveCount(0);
  await expect(replacementAssignment.getByRole("button", { name: "Can’t go", exact: true })).toHaveCount(0);
