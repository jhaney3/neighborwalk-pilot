...
import { Check, MailPlus, Users } from "lucide-react";
...
export function OutingInvitationRoster({ data, eventId, selectedIds, onChange }: Props) {
  ...
  return <section className="outing-invitation-roster" aria-labelledby="outing-invitation-title">
    <div className="walk-crew-section-heading"><div><p>Advance roster</p><h4 id="outing-invitation-title">Who should know about this walk?</h4></div><span>{selectedIds.length} invited</span></div>
    <p className="walk-crew-help">Invite people now so the outing appears on Home and each person can respond. You’ll build the actual target crews from whoever arrives.</p>
    <div className="walk-crew-quick-actions">
      <button type="button" className="button quiet small" onClick={() => onChange(activeVolunteers.map((volunteer) => volunteer.id))}><Users size={15} /> Invite everyone</button>
      <button type="button" className="button quiet small" disabled={!removableInvitationCount} onClick={() => onChange([...checkedInIds])}>{checkedInIds.size ? "Clear other invitations" : "Clear invitations"}</button>
    </div>
    <div className="outing-invitation-list" role="group" aria-label="People invited to this walk">
      {activeVolunteers.map((volunteer) => {
        ...
        return <button type="button" key={volunteer.id} className={invited ? "selected" : ""} aria-pressed={invited} aria-label={checkedIn ? `${volunteer.name} is already checked in` : `${invited ? "Remove" : "Invite"} ${volunteer.name}`} disabled={checkedIn} onClick={() => toggle(volunteer.id)}>
          <span><i>{volunteer.name.charAt(0).toUpperCase()}</i><strong>{volunteer.name}</strong>{status && invited && <small>{statusLabel[status]}</small>}</span>
          {invited ? <Check size={16} aria-hidden="true" /> : <MailPlus size={16} aria-hidden="true" />}
        </button>;
      })}
    </div>
    {savedGroups.length > 0 && <div className="walk-saved-groups"><span>Add a saved group</span><div>{savedGroups.map((team) => <button type="button" key={team.id} onClick={() => addGroup(team.memberIds)}>{team.name}<small>{team.memberIds.filter((id) => activeVolunteers.some((volunteer) => volunteer.id === id)).length} active</small></button>)}</div></div>}
  </section>;
}
