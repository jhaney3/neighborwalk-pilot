...
export function TodayView({ data, activeVolunteerId, canManage, onFollowUps, onPerson, onOuting, onReviewSync, onPeople, onViewMap, onStart, onWalkResponse, additionalAction }: {
  ...
  return <section className="content-view today-view">
    <ViewHeading eyebrow={formatCalendarDate(today, { weekday: "long", month: "long", day: "numeric" })} title={"Hello, " + name + "."} description="Your next walk. The people to remember." aside={canManage && <button type="button" className="button quiet" onClick={onViewMap}><MapPinned size={16} aria-hidden="true" /> View map</button>} />
