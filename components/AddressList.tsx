...
export function AddressList({ data, onOpen, onOpenParcel, onAdd, onTerritoryChange, lockedTerritoryId, targetName, targetParcels, printContext }: {
  ...
  return <section className={`address-list content-view${targetName ? " target-address-list" : ""}${printContext ? " has-field-worksheet" : ""}`} aria-label="Outreach address list">
    <div className="view-heading"><div><p className="eyebrow">{targetName ?? "A map is optional"}</p><h1>{targetName ? "Tonight’s addresses" : "Address list"}</h1><p>{targetName ? `${allEntries.length} ${allEntries.length === 1 ? "location" : "locations"} in this target, including addresses not visited yet.` : "Open a location to record a visit. Saved addresses and restrictions remain available offline."}</p></div>{!targetName && <button className="button primary" onClick={() => setAdding(true)}><Plus size={16} /> Add address manually</button>}</div>
    <div className="list-toolbar">{!lockedTerritoryId && <label>Territory<select value={territory} disabled={action.busy} onChange={(event) => { const value = event.target.value; if (!value) setTerritory(""); else void action.run(() => onTerritoryChange(value), () => setTerritory(value)); }}><option value="">All locations</option>{data.territories.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}</select></label>}<label>Search<input type="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Street address or unit" /></label><button type="button" className="button quiet" onClick={printList}><Printer size={16} /> {printContext ? "Print field worksheet" : "Print address list"}</button></div>
    ...
    {!printContext && <div className="print-heading"><h2>{data.church.name} — outreach addresses</h2><p>Check the app for current restrictions before visiting. Contains no person notes or phone numbers.</p></div>}
