...
export function AddressList({ data, onOpen, onAdd }: {
  ...
  onAdd: (input: { address: string; unit?: string }) => Promise<unknown>;
}) {
  ...
  return <section className="address-list content-view" aria-label="Outreach address list">
    ...
    {adding && <Modal title="Add an address" description="Use only the address needed for outreach. No map lookup is required." onClose={action.busy ? () => undefined : () => setAdding(false)}><form className="form-stack" onSubmit={(e) => { e.preventDefault(); void action.run(() => onAdd({ address: address.trim(), unit: unit.trim() || undefined }), () => { setAddress(""); setUnit(""); setAdding(false); }); }}>
      <p>Added to {data.territories.find((t) => t.id === data.preferences.activeTerritoryId)?.name ?? "the church’s unassigned locations"}.</p>
