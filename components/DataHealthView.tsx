    ...
    <div className="data-health-topline">
      ...
      <section className="data-admin-check" aria-labelledby="data-admin-check-title">
        ...
        <details>
          ...
          <p className="data-help-copy">Signed in with Apple, Google or an email link? Sign out and back in instead.</p>
        </details>
      </section>
    </div>
      ...
      <div className="data-tool-list">
        <details className="data-tool-card">
          ...
          <div className="data-tool-body">
            <h3>Choose a file to download</h3><p>CSV opens in a spreadsheet. JSON is a fuller copy for reviews.</p>
            <div className="data-export-grid">{(["people", "locations", "tasks", "backup"] as const).map((type) => <button type="button" key={type} className="button quiet" disabled={blocked} onClick={() => void run(() => onExport(type), () => setMessage("Downloaded. Keep the file private."))}><Download size={16} /> {type === "backup" ? "Accessible-record JSON" : type[0].toUpperCase() + type.slice(1) + " CSV"}</button>)}</div>
        ...
        <details className="data-tool-card">
          <summary role="button"><span className="data-tool-icon"><Upload size={21} /></span><span><strong>Import a spreadsheet</strong><small>Preview new people or homes before anything is added.</small></span><ChevronDown className="data-tool-chevron" size={19} /></summary>
          <div className="data-tool-body">
            <div className="data-tool-intro"><h3>Import new records in three reviewed steps</h3><p>Imports only add new records. Likely duplicates are skipped. Up to 100 rows and 2 MB.</p></div>
            <ol className="data-import-steps">
              <li><div><strong>Choose the record type and get the template</strong><span>New people start with you as their owner.</span></div><div className="data-import-toolbar"><label>Record type<select value={kind} onChange={(e) => { setKind(e.target.value as ImportKind); setPreview(null); setApproved(false); }}><option value="people">People</option><option value="locations">Locations</option></select></label><button type="button" className="button quiet" onClick={() => downloadBlob(new Blob([csvTemplate(kind)], { type: "text/csv;charset=utf-8" }), "neighborwalk-" + kind + "-import-template.csv")}><FileSpreadsheet size={16} /> Download template</button></div></li>
              <li><div><strong>Choose a completed CSV</strong><span>You’ll see a preview before anything is added.</span></div><label className="data-file-field"><span>UTF-8 CSV file</span><input type="file" accept="text/csv,.csv" disabled={action.busy} onChange={(e) => { const file = e.target.files?.[0]; e.target.value = ""; if (file) void action.run(async () => {
              ...
              {preview && <li><div><strong>Review and approve the rows</strong><span>Nothing has been imported yet.</span></div><div className="data-preview-table"><table><caption>Review each row. Nothing has been imported.</caption><thead><tr><th scope="col">Include</th><th scope="col">Record</th><th scope="col">Review</th></tr></thead><tbody>{preview.rows.map((row) => <tr key={row.line}><td><input type="checkbox" aria-label={"Import CSV row " + row.line} disabled={Boolean(row.problems.length)} checked={selected.includes(row.line)} onChange={(e) => setSelected((lines) => e.target.checked ? [...lines, row.line] : lines.filter((n) => n !== row.line))} /></td><td><strong>Row {row.line} · {row.values.name ?? row.values.address}</strong><dl>{Object.entries(row.values).filter(([key]) => !["name", "address"].includes(key)).map(([key, value]) => <div key={key}><dt>{key.replaceAll("_", " ")}</dt><dd>{value || "Not provided"}</dd></div>)}</dl></td><td>{row.problems.join(" ") || "Valid — select if intended"}</td></tr>)}</tbody></table></div>
                ...
                <button type="button" className="button primary" disabled={blocked || !approved || !selected.length} onClick={() => void run(() => onRun({ action: "import", expectedRevision: previewRevision, operations: csvImportOperations(preview, selected) }), () => { setMessage("Imported."); setPreview(null); setSelected([]); setApproved(false); })}>Import {selected.length} reviewed records</button></li>}
            </ol>
          </div>
        </details>
        <details className="data-tool-card">
          <summary role="button"><span className="data-tool-icon"><GitMerge size={21} /></span><span><strong>Correct records</strong><small>Combine duplicates, correct a conversation or review older records.</small></span>{migrationIssueCount > 0 && <b className="data-tool-count">{migrationIssueCount}</b>}<ChevronDown className="data-tool-chevron" size={19} /></summary>
          <div className="data-tool-body data-correction-tools">
            <details className="data-subtool-card"><summary role="button"><span><strong>Combine duplicate records</strong><small>Keep one record and fold the other into it.</small></span><ChevronDown size={17} /></summary><DuplicateReview data={data} blocked={blocked} error={action.error} onPreview={props.onPreviewDuplicates} onRun={onRun} onAction={run} /></details>
            <details className="data-subtool-card"><summary role="button"><span><strong>Correct a conversation</strong><small>Add a correction. The original stays in the history.</small></span><ChevronDown size={17} /></summary><EncounterCorrectionReview data={data} blocked={blocked} error={action.error} onRun={onRun} onAction={run} /></details>
            <details className="data-subtool-card"><summary role="button"><span><strong>Review older records</strong><small>{migrationIssueCount ? `${migrationIssueCount} ${migrationIssueCount === 1 ? "flag needs" : "flags need"} review.` : "Nothing to review."}</small></span><ChevronDown size={17} /></summary><section className="data-migration-panel" aria-labelledby="historical-data-title"><h3 id="historical-data-title"><History size={19} /> Historical data needing review</h3><p>Marking one reviewed doesn’t change the record.</p>
              ...
              {!data.migrationIssues?.length && <p className="data-empty-note">You’re all caught up.</p>}
            </section></details>
          </div>
        </details>
        <details className="data-tool-card">
          ...
          <div className="data-tool-body">
            <h3>Preview before anything is archived</h3><p>Finds old anonymous conversations and finished follow-ups that nothing depends on.</p>
