        ...
        <details className="data-tool-card">
          ...
          <div className="data-tool-body">
            ...
            <ol className="data-import-steps">
              <li><div><strong>Choose the record type and get the template</strong><span>New people start with you as their owner.</span></div><div className="data-import-toolbar"><label>Record type<select value={kind} onChange={(e) => { setKind(e.target.value as ImportKind); setPreview(null); setApproved(false); }}><option value="people">People</option><option value="locations">Locations</option></select></label><button type="button" className="button quiet" onClick={() => downloadBlob(new Blob([csvTemplate(kind)], { type: "text/csv;charset=utf-8" }), "sendme-" + kind + "-import-template.csv")}><FileSpreadsheet size={16} /> Download template</button></div></li>
