...
export function RecoveryView({ data, online, onPreview, onResolve, onExport, onArchives, onDownloadArchive, onSync }: {
  ...
  onExport: () => void; onArchives: () => Promise<Archive[]>; onDownloadArchive: (key: string) => Promise<void>; onSync: () => Promise<boolean>;
}) {
  ...
  return <section className="content-view recovery-view">
    ...
    <div className="today-card"><h2>{data.sync.legacyRecoveryRequired ? "Previous-version work needs a careful review" : first?.state === "needs_review" ? "A change needs your decision" : commands.length ? commands.length + " transactions waiting to share" : "No queued transactions on this device"}</h2>
      ...
      <p>Exports contain sensitive church records that this account can access. Store them securely; do not send them through public channels.</p>
    ...
    <section className="today-card"><h2><ShieldCheck size={22} /> Preserved recovery archives</h2><p>Archives stay on this device under this account and church. Clearing browser storage removes them; download a copy before doing so.</p><button className="button quiet" disabled={action.busy} onClick={() => void action.run(async () => setArchives(await onArchives()))}>Show this account’s archives</button><ul>{archives.map((archive) => <li key={archive.key}><p>{new Date(archive.createdAt).toLocaleString()} · {archive.reason}</p><button className="button quiet" onClick={() => void action.run(() => onDownloadArchive(archive.key))}>Download preserved work</button></li>)}</ul></section>
