begin;
-- Extend the command whitelist without reopening any direct browser writes.
...
begin
  if kind='event' then
    ...
    if old_status in ('completed','cancelled','declined') and next_status is distinct from old_status then
...
-- New task links must never reach a different tenant. NOT VALID preserves any
-- historical inconsistencies for the migration review instead of deleting them.
alter table public.outreach_tasks add constraint outreach_tasks_parent_tenant_fk
  foreign key(church_id,parent_task_id) references public.outreach_tasks(church_id,id) not valid;
create index outreach_tasks_parent_idx on public.outreach_tasks(church_id,parent_task_id);

-- Group defaults now refer to normalized reusable groups, not snapshot JSON.
alter table public.conversation_guide_team_defaults add constraint conversation_guide_team_normalized_fk
  foreign key(church_id,team_id) references public.outreach_teams(church_id,id) not valid;
comment on table public.conversation_guide_team_defaults is 'Optional church-guide defaults for normalized, reusable outreach groups.';
commit;
