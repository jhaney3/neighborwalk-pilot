...
begin
  ...
  if coalesce(contact_kind, '') not in ('email','phone') or contact is null or length(contact) > 320
    ...
    raise exception 'Enter a valid email or international phone number and access level.' using errcode = '22023';
  end if;
  if (select count(*) from private.shared_invitations i where i.church_id = church and i.created_by = auth.uid() and i.created_at > now() - interval '1 hour') >= 25
    ...
    raise exception 'Invitation limit reached. Review pending links or try later.' using errcode = '54000';
  end if;
  update private.shared_invitations i set revoked_at = now()
    where i.church_id = church and i.contact_kind = create_shared_invitation.contact_kind and i.contact_value = contact
  ...
  insert into private.shared_invitations(church_id, contact_kind, contact_value, recipient_name, role, token_hash, created_by)
    values(church, contact_kind, contact, btrim(coalesce(recipient_name, '')), invitation_role, encode(extensions.digest(token, 'sha256'), 'hex'), auth.uid())
...
begin
  ...
  if invitation.accepted_by = actor and invitation.accepted_at is not null then
    if not exists(select 1 from public.church_memberships m where m.church_id = church and m.user_id = actor and m.active) then
      raise exception 'Ask a leader to review your access.' using errcode = '42501';
    end if;
  ...
  if invitation.revoked_at is not null or invitation.accepted_at is not null or invitation.expires_at <= now() then
    raise exception 'This invitation expired, was revoked, or has already been used. Ask your leader for a new link.' using errcode = '22023';
  end if;
  ...
  if exists(select 1 from public.church_memberships m where m.user_id = actor and (m.active or m.church_id = church)) then
    raise exception 'This account already has a workspace membership. Ask a leader to review access.' using errcode = '23505';
  end if;
