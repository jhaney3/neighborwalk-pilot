...
export function reviewedEncounter(visit: Visit) {
  const latest = visit.corrections?.at(-1);
...
export function lastRecordedContact(data: NeighborWalkData, personId: string): { at: string; source: "encounter" | "historical" } | undefined {
  ...
  const historical = data.residents.filter((person) => family.has(person.id)).map((person) => person.lastContactAt).filter((date): date is string => Boolean(date)).sort().at(-1);
