...
export function createSeedData(): NeighborWalkData {
  ...
  const visits = properties
    ...
    .map((property, index) => ({
      ...
      followUpConsent: property.currentOutcome === "follow_up",
  ...
  return {
    ...
    guide: [
      ...
      },
      {
        ...
        reminder: "Record details only when the person clearly gives permission.",
...
export function createWorkspaceData(churchName: string, user: { id: string; email: string }): NeighborWalkData {
  const now = new Date();
  const endsAt = new Date(now);
  endsAt.setDate(endsAt.getDate() + 30);
  const demo = createSeedData();
  const churchId = `church_${user.id.replaceAll("-", "")}`;
  const volunteerId = `volunteer_${user.id.replaceAll("-", "")}`;
  const eventId = `event_${now.getTime()}`;
  const territoryId = `territory_lawrenceburg_${now.getTime()}`;
  const center: Coordinates = [-87.3347, 35.2423];
  const emailName = user.email.split("@")[0]?.replace(/[._-]+/g, " ").trim() || "Church leader";
  const displayName = emailName.replace(/\b\w/g, (letter) => letter.toUpperCase());
  ...
  return {
    ...
    volunteers: [{ id: volunteerId, churchId, name: displayName, email: user.email, role: "leader", active: true }],
    events: [{
      id: eventId,
      churchId,
      name: "Lawrenceburg neighborhood outreach",
    ...
    territories: [{
      ...
      churchId,
      eventId,
      name: "Lawrenceburg pilot",
      color: "#286c59",
      center,
      zoom: 15.7,
      boundary: boundary(center, 0.008, 0.006),
    }],
    teams: [],
    properties: [],
    visits: [],
