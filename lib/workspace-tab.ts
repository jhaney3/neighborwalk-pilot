...
export function acquireWorkspaceTab(locks: Pick<LockManager, "request"> | undefined, key: string): Promise<() => void> {
  if (!locks) return Promise.reject(new Error("This browser cannot safely coordinate saved work across tabs. Open SendMe in an up-to-date browser; existing device work has not been cleared."));
